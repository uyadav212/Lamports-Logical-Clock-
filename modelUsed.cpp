...
*
*			Here we are defining all the funcions of all classes which are used to implement Lamport's logical clock.
*/
...
void Process::updateClock(int timestamp, int d) {

	clock++;
	//to cover the scenario when we receive msg with timestamp
	//			clock = max(clock, timestamp+d)
	if ((timestamp + d) > clock)
		clock = timestamp + d;
}

void Process::updateClock() { clock++;}
...
void Process::sendMsgEvent(string to, string msg, int transmitionDelay) {
	...
	msgToSend->d = transmitionDelay;

	cout << ": " << msg << "\t [Sent by process " << processId << " to process " << to << " at timestamp: " << clock << "]\n";
...
void Process::printMsgEvent(string msg) {
	...
	cout << ": " << msg << "\t [Printed by process " << processId << " at timestamp: " << clock << "]\n";
}
...
bool Process::receiveMsgEvent(string from, string msg) {
	...
	if (foundAt == channelMsgPool.end()) {
		...
		if (recMsg->timestamp != -1)
			updateClock(recMsg->timestamp, recMsg->d);
		else
			updateClock();

		//if clock time is more then tmpGlobalClock then this event will be executed later, to maintain causality
		if (clock > tmpGlobalClock) {
			recMsg->timestamp = -1;			//to indicate time is already calculated
			clock--;
			return false;
		}
		...
		cout << ": " << recMsg->msgContent << "\t [Received by process " << processId << " from process " 
			<< recMsg->fromProcess << " at timestamp: " << clock << "]\n";
