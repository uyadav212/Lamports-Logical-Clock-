...
*
*			Write a program in C++ / Java to implement Lamport’s logical clocks. Your program should take as input a description of 
...
string chooseProcessToExec() {
	vector<string>::iterator processIt;
	bool canIncrementGlobalClock = false;
	...
	for (processIt = allProcessId.begin(); processIt < allProcessId.end(); processIt++) {
		...
		if (isCompletedProcess == completedProcessId.end()) {
			//we can execute any process which is 
			//					1. not in activeProcess list OR
			//					2. if it is in activeProcess list then it should not be blocked or ended.
			//			AND	 clock value of process is less then global tmp clock
			Process *processPointer = findProcessForPID(*processIt);
			if (processPointer == NULL)
				return *processIt;

			if (!(processPointer->getBlocked()) && !(processPointer->getEnded())) {
				if (processPointer->getClock() < tmpGlobalClock)
					return *processIt;
				else
					canIncrementGlobalClock = true;	
			}
		}
	}

	//found no process which is not blocked and can be executed, now will check for deadlock and unblock processes
	//whose recv message is present in the channel message pool
	if (canIncrementGlobalClock)
		tmpGlobalClock++;
	else
...
int main() {
	...
	//this will keep track of global timestamp [in order to maintain causality]
	tmpGlobalClock = 1;
